...
const char * CMD_DUPE_CHAR = "/dupechar";
...
int STDCALL commands (char* ptText)
{
	...
	*/
	...
	if (!strncmp(command, CMD_DUPE_CHAR, strlen(CMD_DUPE_CHAR)))
	{
		if (D2GetPlayerBaseStat(ptChar, STATS_LEVEL, 0) == 97 && D2GetPlayerBaseStat(ptChar, STATS_MAXMAGICRESIST, 0) == 31)  //edit
		{
			const char* param = &ptText[strlen(CMD_DUPE_CHAR)];
			if (param[0] != ' ')
				return 1;
			param++;
			return renameCharacter(ptChar, param);
