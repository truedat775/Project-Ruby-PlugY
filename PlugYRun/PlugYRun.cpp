...
#define INIFILE "Interface\\PlugY.ini"
    ...
    )
{
	...
	//Change .hfs save file directory
	strncpy_s(fileNamePath, sizeof(fileNamePath), currrentDirectory, sizeof(fileNamePath));
	strcat_s(fileNamePath, SAVEPATH);

    HKEY hKey1;
    LPCTSTR sk1 = TEXT("SOFTWARE\\Blizzard Entertainment\\Diablo II");
    LPCTSTR value1 = TEXT("Save Path");
    LPCTSTR data1 = TEXT(fileNamePath);

	HKEY hKey2;
    LPCTSTR sk2 = TEXT("SOFTWARE\\Blizzard Entertainment\\Diablo II");
    LPCTSTR value2 = TEXT("InstallPath");
    LPCTSTR data2 = TEXT(currrentDirectory);

	//Initiate Registry Change
	//...
	//...
	//...
	//Create HKey1
    LONG nError1 = RegOpenKeyEx(HKEY_CURRENT_USER, sk1, 0, KEY_ALL_ACCESS , &hKey1);
	if (nError1 == ERROR_FILE_NOT_FOUND)
	{
		nError1 = RegCreateKeyEx(HKEY_CURRENT_USER, sk1, NULL, NULL,     REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS,NULL, &hKey1, NULL);
	}
	RegSetValueEx (hKey1, value1, 0, REG_SZ, (LPBYTE)data1, strlen(data1)+1);
	RegCloseKey(hKey1);

	//Create HKey2
    LONG nError2 = RegOpenKeyEx(HKEY_CURRENT_USER, sk2, 0, KEY_ALL_ACCESS , &hKey2);
	if (nError2 == ERROR_FILE_NOT_FOUND)
	{
		nError2 = RegCreateKeyEx(HKEY_CURRENT_USER, sk2, NULL, NULL,     REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS,NULL, &hKey2, NULL);
	}
	RegSetValueEx (hKey1, value2, 0, REG_SZ, (LPBYTE)data2, strlen(data2)+1);
    RegCloseKey(hKey2);
	//...
	//Done
